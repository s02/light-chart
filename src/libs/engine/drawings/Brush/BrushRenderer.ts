...
export class BrushRenderer implements IPrimitivePaneRenderer {
  #points: Point[]
  #params: BrushParams

  constructor(points: Point[], params: BrushParams) {
    this.#points = points
    this.#params = params
  }
  ...
  draw(target: CanvasRenderingTarget2D) {
    target.useBitmapCoordinateSpace((scope) => {
      ...
      if (pts.length < 2) {
        if (pts.length === 1) {
          ctx.beginPath()
          ctx.arc(pts[0].x * hpr, pts[0].y * vpr, (this.#params['line-width'] / 2) * hpr, 0, Math.PI * 2)
