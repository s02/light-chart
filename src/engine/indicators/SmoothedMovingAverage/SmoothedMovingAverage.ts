...
export class SmoothedMovingAverage extends AbstractIndicator implements Indicator {
  ...
  constructor(chart: IChartApi, datafeed: Datafeed, options: IndicatorOptions) {
    ...
    this.#series = this.#chart.addSeries(
      LineSeries,
      { ...COMMON_SERIES_SETTINGS, ...this.#params['smma-line'], priceLineVisible: false },
      this.paneIndex
    )
  }
  ...
  setParams(params: StudyParams) {
    ...
    this.#series.applyOptions(this.#params['smma-line'])
  }
  ...
  getLegend(seriesData: SeriesMap) {
    ...
    if (data) {
      legend.data.push({ value: formatPrice((data as LineData<Time>).value), color: this.#params['smma-line'].color })
    }
    return legend
  }
