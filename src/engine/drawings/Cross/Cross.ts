import { BaseDrawing } from '../BaseDrawing'
import { resolveStudyParams } from '@engine/schema'
import { CrossPaneView } from '@engine/drawings/Cross/CrossPaneView'
import type { InferStudyValues, StudyParams, StudySchema } from '@engine/schema'
import type { DrawingOptions } from '@engine/drawings/types'
import type { IChartApi, Point } from 'lightweight-charts'

const CROSS_SCHEMA = {
  text: [],
  inputs: [],
  style: [
    { type: 'line-color', key: 'line-color', default: 'rgb(255 152 0)', fastPanel: true },
    { type: 'line-width', key: 'line-width', default: 2, fastPanel: true },
    { type: 'line-style', key: 'line-style', default: 'solid', fastPanel: true }
  ]
} as const satisfies StudySchema

export type CrossParams = InferStudyValues<typeof CROSS_SCHEMA.inputs> & InferStudyValues<typeof CROSS_SCHEMA.style>

export class Cross extends BaseDrawing {
  static readonly ikey = 'cross' as const
  static readonly points = 1

  #params: CrossParams

  constructor(chart: IChartApi, options?: DrawingOptions) {
    super(chart)
    this.#params = resolveStudyParams(CROSS_SCHEMA.inputs, CROSS_SCHEMA.style, CROSS_SCHEMA.text, options?.params)
  }

  override setParams(params: StudyParams) {
    this.#params = resolveStudyParams(CROSS_SCHEMA.inputs, CROSS_SCHEMA.style, CROSS_SCHEMA.text, params)
    if (this.requestUpdate) {
      this.requestUpdate()
    }
  }

  override getSchema() {
    return {
      ikey: Cross.ikey,
      schema: CROSS_SCHEMA,
      params: this.#params
    }
  }

  override paneViews() {
    const viewport = this.getViewport()

    if (viewport && this.anchors.length === 1) {
      return [new CrossPaneView(viewport, this.anchors[0], this.anchorsVisible, this.#params)]
    }

    return []
  }

  override checkTap(point: Point): boolean {
    const viewport = this.getViewport()
    if (!viewport) return false

    const p = viewport.anchorToPoint(this.anchors[0])
    if (!p) return false

    return Math.abs(point.x - p.x) <= Cross.hitThreashold || Math.abs(point.y - p.y) <= Cross.hitThreashold
  }
}
