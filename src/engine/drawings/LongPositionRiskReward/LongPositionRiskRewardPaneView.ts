...
export class LongPositionRiskRewardPaneView extends RiskRewardPaneView implements IPrimitivePaneView {
  ...
  constructor(
    ...
    this.targetPriceOffset = this.params['rr-target-price'] - this.params['rr-entry-price']
    this.stopPriceOffset = this.params['rr-entry-price'] - this.params['rr-stop-price']

    const closePrice = this.closePrice()
    this.openPnl = closePrice ? closePrice - this.params['rr-entry-price'] : 0
  }
  ...
  renderer() {
    ...
    return new RiskRewardRenderers(
      new RwRectangleRenderer(p0, p1, p2, p3, this.#withDots, {
        upColor: this.params['rr-profit-fill'],
        downColor: this.params['rr-loss-fill']
      }),
