...
import type { HttpTransport, Quote, WsTransport } from './types'
...
export class WebSocketClient implements WsTransport {
  ...
  //#host = 'dev-ws.bindev.info'
  #host = 'ws.binarium.com'

  constructor(http: HttpTransport) {
    this.#ws = http.getWebsocketToken().then((token) => {
      const ws = new Centrifuge(`wss://${this.#host}/connection/websocket`, {
        debug: false,
...
export const WS_CLIENT_KEY: InjectionKey<WebSocketClient> = Symbol('ws')
